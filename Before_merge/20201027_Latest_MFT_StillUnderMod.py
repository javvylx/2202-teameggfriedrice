...
def csvheader():
    ...
    "Base Record File Reference", "Next Attribute ID", "MFT Data Number", "$SI Creation Time", "$SI Modified Time", 
    "$SI Change Time", "$SI Access Time", "$FN_1 Creation Time", "$FN_1 Modified Time", 
    "$FN_1 Change Time", "$FN_1 Access Time", "$FN_1 Filename Length", "$FN_1 Name Space Length", "$FN_1 Filename", 
    "$FN_2 Creation Time", "$FN_2 Modified Time", "$FN_2 Change Time", "$FN_2 Access Time", "$FN_2 Filename Length", "$FN_2 Name Space Length", "$FN_2 Filename", 
    "$FN_3 Creation Time", "$FN_3 Modified Time", "$FN_3 Change Time", "$FN_3 Access Time", "$FN_3 Filename Length", "$FN_3 Name Space Length", "$FN_3 Filename"]
    return listheader
...
def get_mft_eh_val(file):
    ...
    '''
    #******************************************************************************************************************************************#
    overalldata.extend([count,anonamlymftrecord,anomaly_SI_1,anomaly_SI_2,anomaly_SI_3,anomaly_SI_4,anomaly_SI_5,anomaly_SI_6,anomaly_SI_FN_1,anomaly_SI_FN_2,anomaly_SI_FN_3,anomaly_SI_FN_4])
    #******************************************************************************************************************************************#
    '''
    ...
    if mft_record['fncount'] == 0:
        ...
        for i in range(3):
            if (i) == mft_record['fncount']-1:
                break
            check = i + 1
            csvcontent.extend([excel_date(mft_record['fn',check]['crtime'][1]),excel_date(mft_record['fn',check]['mtime'][1]),
                            excel_date(mft_record['fn',check]['ctime'][1]),excel_date(mft_record['fn',check]['atime'][1]),
                            mft_record['fn',check]['nlen'],mft_record['fn',check]['nspace'], str(mft_record['fn',check]['name'])])
