import binascii, json, ctypes, struct, os, sys, getopt, datetime, csv, hashlib
...
def anomaly_timestamp_check_SI_FN(mft_record, fn_count):
    ...
    3: $SI (crtime) after $FN (crtime)
    4: $FN after Current Time
    '''
    check = 1
...
def get_mft_eh_val(file):
    result = open("./output/log.csv", "w")  # ! <-- txt file dir to be changed
    count = 0
    overalldata, wd = [], []
    ...
    anomaly_SI_FN_1, anomaly_SI_FN_2, anomaly_SI_FN_3, anomaly_SI_FN_4 = 0, 0, 0, 0
    anonamlymftrecord = 0
    while (True):
        count += 1
        ...
        if hexdata != "":
            ...
            if mft_record['fncount'] > 0 and mft_record[
                'fn', 1] and 'si' in mft_record and anomaly_SI_1 == 0 and anomaly_SI_2 == 0:
    ...
    anomaly_SI_FN_3 : $SI (crtime) after $FN (crtime)
    anomaly_SI_FN_4 : $FN after Current Time
    '''
    # ******************************************************************************************************************************************#
    overalldata.extend(
        [count, anonamlymftrecord, anomaly_SI_1, anomaly_SI_2, anomaly_SI_3, anomaly_SI_4, anomaly_SI_5, anomaly_SI_6,
         anomaly_SI_FN_1, anomaly_SI_FN_2, anomaly_SI_FN_3, anomaly_SI_FN_4])
    # ******************************************************************************************************************************************#
    '''
    ...
    # print("Anomaly $SI + $FN: ", anomaly_SI_FN_1, anomaly_SI_FN_2, anomaly_SI_FN_3, anomaly_SI_FN_4)
...
# ************************************************ Start of Image Summary ******************************************** #
    ...
    document.add_paragraph("The total records in the image file is: " + str(count), style=None)
    document.add_paragraph("The sha256sum of the image file is: " + str(fileHash(file.name)), style=None)
    document.add_paragraph("The size of the image file is: " + str(fileSize(file.name)), style=None)
...
# ************************************************ Start of Anomalies Summary **************************************** #
    ...
    anomaly_records = (
        ...
        (2, '0s in $FN anomaly', anomaly2),
        (3, 'Invalid $SI Timestamp', anomaly3),
        (4, '$SI nanosec = 0', anomaly4),
        (5, 'SI (crtime) after $SI (mtime)', anomaly5),
        (6, '$SI (atime) after $SI (mtime) and $SI (atime) after $SI (crtime)', anomaly6),
        (7, '$SI after Current Time', anomaly7),
        (8, '0s in $FN', anomaly8),
        (9, 'Invalid $FN Timestamp', anomaly9),
        (10, '$SI (crtime) after $FN (crtime)', anomaly10),
        (11, '$FN after Current Time', anomaly11)
    )
...
# ************************************************ Start of Flagged Anomalies **************************************** #
    summary = document.add_heading('Flagged Anomalies', 0)
    summary.alignment = WD_PARAGRAPH_ALIGNMENT.LEFT
    d = {}
    print("\nPlease wait... Inserting all anomalies found into csv files...")
    for tup in wd:
        record_name = tup[0]
        anonamly = tup[1]
        if anonamly == "anomaly_SI_1":
            anonamly = "0s in $FN"
        if anonamly == "anomaly_SI_2":
            anonamly = "Invalid $SI Timestamp"
        if anonamly == "anomaly_SI_3":
            anonamly = "$SI nanosec = 0"
        if anonamly == "anomaly_SI_4":
            anonamly = "$SI (crtime) after $SI (mtime)"
        if anonamly == "anomaly_SI_5":
            anonamly = "$SI (atime) after $SI (mtime) and $SI (atime) after $SI (crtime)"
        if anonamly == "anomaly_SI_6":
            anonamly = "$SI after Current Time"
        if anonamly == "anomaly_SI_FN_1":
            anonamly = "0s in $FN"
        if anonamly == "anomaly_SI_FN_2":
            anonamly = "Invalid $FN Timestamp"
        if anonamly == "anomaly_SI_FN_3":
            anonamly = "$SI (crtime) after $FN (crtime)"
        if anonamly == "anomaly_SI_FN_4":
            anonamly = "$FN after Current Time"

        try:
            # Generating all anomalies in a CSV file
            with open("./output/overall_anomalies.csv", 'a', newline='', encoding='utf-8') as fd:
                wr = csv.writer(fd, dialect='excel')
                wr.writerow([record_name, anonamly])
            d[record_name] = anonamly
        except:
            continue

    table = document.add_table(rows=1, cols=2, style='TableGrid')
    hdr_cells = table.rows[0].cells
    hdr_cells[0].text = 'Record Number'
    hdr_cells[1].text = 'Anomaly Description'
    counter = 0
    for rn, anomalydesc in d.items():
        row_cells = table.add_row().cells
        row_cells[0].text = str(rn)
        row_cells[1].text = anomalydesc
        counter = counter + 1
        if counter == 100:  # Due to limitation, 100 records are being generated
            break

# ************************************************ End of Flagged Anomalies ****************************************** #
# *********************************************** Creation of Footer ************************************************* #
...
def writecsv(mft_record):
    ...
    if 'si' not in mft_record:
        csvcontent.extend(padding(4))
    elif 'si' in mft_record and mft_record['si']['crtime'] is not None:
        csvcontent.extend([excel_date(mft_record['si']['crtime'][1]), excel_date(mft_record['si']['mtime'][1]),
                           excel_date(mft_record['si']['ctime'][1]), excel_date(mft_record['si']['atime'][1])])
    if mft_record['fncount'] == 0:
        ...
        for i in range(3):
            ...
            check = i + 1
            csvcontent.extend(
                [excel_date(mft_record['fn', check]['crtime'][1]), excel_date(mft_record['fn', check]['mtime'][1]),
                 excel_date(mft_record['fn', check]['ctime'][1]), excel_date(mft_record['fn', check]['atime'][1]),
                 mft_record['fn', check]['nlen'], mft_record['fn', check]['nspace'],
                 str(mft_record['fn', check]['name'])])
