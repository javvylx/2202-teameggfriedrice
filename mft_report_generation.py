...
def get_mft_eh_val(file):
    gen_folderpath = r'./output/' + time.strftime("%Y-%m-%d_%H-%M-%S",
                                                  time.localtime()) + "/"  # Generating Folder Path with Date Time Stamps
    os.makedirs(gen_folderpath, exist_ok=True)  # Go ahead with generation even if folder exists

    # Save file output to folder generated by date/time
...
def createReport(count, file, overalldata, wd, reportName, gen_folderpath):
    ...
    print("\nGenerating PDF Document in: " + gen_folderpath + reportName + ".pdf\n")
...
def args_route(mftfn):
    if (mftfn == None):
        ...
        f = open(mftfn, "rb")
        print("Extracting Master File Table (MFT) Records! Please wait...")
        get_mft_eh_val(f)
